...
void SquareFall::setup(){
	...
	for (int i = 0; i < NUM; i++) {
		for (int j = 0; j < 2; j++) {
			...
			ypoint[i][j] = 0; //ofGetHeight() - ofRandom(200, 500) + j * 400;
		}
	}
...
//--------------------------------------------------------------
void SquareFall::update(){

	for (int i = 0; i < NUM; i++) {
		for (int j = 0; j < 2; j++) {
			if (ypoint[i][j] >= ofGetHeight()) {
				ypoint[i][j] = 0;//ofGetHeight() + 30 * 6 + j * 400;
				height[i] = ofRandom(30, 15);
				if (j == 1)
...
void SquareFall::draw(){
	...
	for (int i = 0; i < NUM ; i++) {
		for (int j = 0; j < 6; j++) {
			for (int k = 0; k < 2; k++) {
				box.set(80);
				box.setPosition(xpoint[i], ypoint[i][k], 0);
